...
const BalancePage = () => {
      ...
      try {
        ...
        const enrichedBalances: BalanceDto[] = rawBalances.map((item) => ({
          ...
          resourceName:
            ...
            "Неизвестно",
          unitName:
            units.find((u) => u.id === item.unitOfMeasureId)?.name ||
            "Неизвестно",
        }));
  ...
  const handleResourceFilterChange: React.ChangeEventHandler<
    HTMLInputElement
  > = (e) => {
    const value = e.target.value;
    setResourceFilter(typeof value === "string" ? [parseInt(value)] : value);
  };

  const handleUnitFilterChange: React.ChangeEventHandler<HTMLInputElement> = (
    e
  ) => {
    const value = e.target.value;
    setUnitFilter(typeof value === "string" ? [parseInt(value)] : value);
  };
